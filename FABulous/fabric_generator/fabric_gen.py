...
class FabricGenerator:
            ...
            if muxSize == 0:
                ...
                if self.fabric.multiplexerStyle == MultiplexerStyle.CUSTOM:
                    if muxSize == 2:
                        ...
                        for i in range((muxSize+numGnd).bit_length()-1):
                            portsPairs.append(
    ...
    def generateTile(self, tile: Tile) -> None:
        ...
        ########## SAUBER >
        self.writer.addPortScalar("rst", IO.INPUT, indentLevel=2)
        self.writer.addPortScalar("rsto", IO.OUTPUT, indentLevel=2)
        ########## SAUBER <
    ...
    def generateFabric(self) -> None:
        ...
        for y, row in enumerate(self.fabric.tile):
            for x, tile in enumerate(row):
                ...
                ########## SAUBER >
                self.writer.addConnectionScalar(f"Tile_X{x}Y{y}_rsto")
                ########## SAUBER <
        ...
        for y, row in enumerate(self.fabric.tile):
            for x, tile in enumerate(row):
                ...
                if not superTile:
                    ...
                    if y + 1 < self.fabric.numberOfRows and self.fabric.tile[y+1][x] != None:
                        portsPairs.append(
                            ("rst", f"Tile_X{x}Y{y+1}_rsto"))
                    else:
                        portsPairs.append(("rst", "rst"))

                    # for rsto
                    portsPairs.append(("rsto", f"Tile_X{x}Y{y}_rsto"))
    ...
    def generateTopWrapper(self) -> None:
        ...
        ########## SAUBER >
        # SAUBER settings 
        dr_check_GADGETS = False
        rng_ratio = 3

        rows_nr = self.fabric.numberOfRows - 2
        masked_range_columns = 77 #self.fabric.numberOfColumns - 1 (- ctrl_part)  
        ctrl_range_columns = self.fabric.numberOfColumns - 2 - masked_range_columns
        rng_nr = rows_nr * (masked_range_columns//rng_ratio)

        f_signals_masked = rows_nr - 1
        f_signals_ctrl = rows_nr - 1

        if (dr_check_GADGETS):
            f_signals_masked = (masked_range_columns * rows_nr) - 1
            f_signals_ctrl = (ctrl_range_columns * rows_nr) -1    

        # adjusted ports IO
        self.writer.addPortVector(
            "I_top_0_t", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "I_top_0_f", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "I_top_1_t", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "I_top_1_f", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "T_top", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "O_top_0_t", IO.INPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "O_top_0_f", IO.INPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "O_top_1_t", IO.INPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "O_top_1_f", IO.INPUT, f"{numberOfRows}-1", indentLevel=2)
        
        # ctrl_IO
        self.writer.addPortVector(
            "ctrl_I_top_0_t", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "ctrl_I_top_0_f", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "ctrl_T_top", IO.OUTPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "ctrl_O_top_0_t", IO.INPUT, f"{numberOfRows}-1", indentLevel=2)
        self.writer.addPortVector(
            "ctrl_O_top_0_f", IO.INPUT, f"{numberOfRows}-1", indentLevel=2)

        # additional custom ports   
        self.writer.addNewLine()
        self.writer.addComment("Custom ports (*SAUBER*)",
                               onNewLine=True, indentLevel=2)
        #reset
        self.writer.addPortScalar("rst", IO.INPUT, indentLevel=2)
        self.writer.addPortScalar("f_detected", IO.OUTPUT, indentLevel=2)
        self.writer.addPortScalar("prech1", IO.OUTPUT, indentLevel=2)
        self.writer.addPortScalar("prech2", IO.OUTPUT, indentLevel=2) 

        self.writer.addPortScalar("prng_rst", IO.INPUT, indentLevel=2)          
        self.writer.addPortVector("key_t", IO.INPUT, "79", indentLevel=2)
        self.writer.addPortVector("key_f", IO.INPUT, "79", indentLevel=2)
        self.writer.addPortVector("iv_t", IO.INPUT, "79", indentLevel=2)
        self.writer.addPortVector("iv_f", IO.INPUT, "79", indentLevel=2)
        self.writer.addNewLine()
        ...
        # wires
        self.writer.addNewLine()
        self.writer.addComment("Custom signal declarations for SAUBER", onNewLine=True)      
        self.writer.addConnectionVector(
            "F_masked1_top", f"{f_signals_masked}")
        self.writer.addConnectionVector(
            "F_masked2_top", f"{f_signals_masked}")
        self.writer.addConnectionVector(
            "F_ctrl_top", f"{f_signals_ctrl}")        
        self.writer.addConnectionVector(
            "R_t_top", f"{rng_nr-1}")
        self.writer.addConnectionVector(
            "R_f_top", f"{rng_nr-1}")
        ...
        for i in range(1, self.fabric.numberOfRows - 1):
            for j in range(rng_ratio,  masked_range_columns+1, rng_ratio):
                portList.append(f"Tile_X{j}Y{i}_R_t")
                signal.append(f"R_t_top[{rng_ij}]")
                portList.append(f"Tile_X{j}Y{i}_R_f")
                signal.append(f"R_f_top[{rng_ij}]")
                rng_ij += 1
        ...
        # DR check result
        #for i in range(1, self.fabric.numberOfRows - 1):
        #    portList.append(f"Tile_X0Y{i}_A_DR_fault")
        #    signal.append(f"f_detected")
        #    portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_DR_fault")
        #    signal.append(f"f_detected")
        ...
        F_ij = 0
        for i in range(1, self.fabric.numberOfRows - 1):
            portList.append(f"Tile_X0Y{i}_A_F_masked1")
            signal.append(f"F_masked1_top[{F_ij}]")
            portList.append(f"Tile_X0Y{i}_A_F_masked2")
            signal.append(f"F_masked2_top[{F_ij}]")
            F_ij += 1 

        # DR check ctrl IO
        F_ij = 0
        for i in range(1, self.fabric.numberOfRows - 1):
            portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_F_ctrl")
            signal.append(f"F_ctrl_top[{F_ij}]")
            F_ij += 1

        if (dr_check_GADGETS):
            # DR check ctrl gadgets
            for i in range(1, self.fabric.numberOfRows - 1):
                for j in range(1,  ctrl_range_columns+1):
                    jj = j + masked_range_columns + 1
                    portList.append(f"Tile_X{jj}Y{i}_F_ctrl")
                    signal.append(f"F_ctrl_top[{F_ij}]")
                    F_ij += 1  

            # DR check secure gadgets
            for i in range(1, self.fabric.numberOfRows - 1):
                for j in range(1,  masked_range_columns+1):
                    portList.append(f"Tile_X{j}Y{i}_F_masked1")
                    signal.append(f"F_masked1_top[{F_ij}]")
                    portList.append(f"Tile_X{j}Y{i}_F_masked2")
                    signal.append(f"F_masked2_top[{F_ij}]")
                    F_ij += 1  
        #####
        ...
        # prech
        for i in range(1, self.fabric.numberOfRows - 1):
            portList.append(f"Tile_X0Y{i}_A_prech1")
            signal.append(f"prech1")
            portList.append(f"Tile_X0Y{i}_A_prech2")
            signal.append(f"prech2")      
            portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_prech2")
            signal.append(f"prech2")  
        ...
        for i in range(1, self.fabric.numberOfRows - 1):
            ...
            portList.append(f"Tile_X0Y{i}_A_I_top_0_t")
            signal.append(f"I_top_0_t[{i-1}]")
            portList.append(f"Tile_X0Y{i}_A_I_top_0_f")
            signal.append(f"I_top_0_f[{i-1}]")
            portList.append(f"Tile_X0Y{i}_A_I_top_1_t")
            signal.append(f"I_top_1_t[{i-1}]")
            portList.append(f"Tile_X0Y{i}_A_I_top_1_f")
            signal.append(f"I_top_1_f[{i-1}]")

        #signal += [f"I_top[{i}]" for i in range(numberOfRows*4-1, -1, -1)]

        for i in range(1, self.fabric.numberOfRows - 1):
            #portList.append(f"Tile_X0Y{i}_A_T_top")
            #portList.append(f"Tile_X0Y{i}_B_T_top")
            portList.append(f"Tile_X0Y{i}_A_T_top")
            signal.append(f"T_top[{i-1}]")

        #signal += [f"T_top[{i}]" for i in range(numberOfRows-1, -1, -1)]

        for i in range(1, self.fabric.numberOfRows - 1):
            #portList.append(f"Tile_X0Y{i}_A_O_top")
            #portList.append(f"Tile_X0Y{i}_B_O_top")
            portList.append(f"Tile_X0Y{i}_A_O_top_0_t")
            signal.append(f"O_top_0_t[{i-1}]")
            portList.append(f"Tile_X0Y{i}_A_O_top_0_f")
            signal.append(f"O_top_0_f[{i-1}]")
            portList.append(f"Tile_X0Y{i}_A_O_top_1_t")
            signal.append(f"O_top_1_t[{i-1}]")
            portList.append(f"Tile_X0Y{i}_A_O_top_1_f")
            signal.append(f"O_top_1_f[{i-1}]")            

        #signal += [f"O_top[{i}]" for i in range(numberOfRows*4-1, -1, -1)]

        # ctrl_IO
        for i in range(1, self.fabric.numberOfRows - 1):
            portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_I_top_0_t")
            signal.append(f"ctrl_I_top_0_t[{i-1}]")
            portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_I_top_0_f")
            signal.append(f"ctrl_I_top_0_f[{i-1}]")

            portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_T_top")
            signal.append(f"ctrl_T_top[{i-1}]")

            portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_O_top_0_t")
            signal.append(f"ctrl_O_top_0_t[{i-1}]")
            portList.append(f"Tile_X{self.fabric.numberOfColumns-1}Y{i}_A_O_top_0_f")
            signal.append(f"ctrl_O_top_0_f[{i-1}]")
        ...
        self.writer.addInstantiation(compName=f"fault_detector  #(.Nm1({f_signals_masked}), .Nm2({f_signals_masked}), .Nctrl({f_signals_ctrl}))",
                                         compInsName=f"DR_check",
                                         portsPairs=portsPairs1)
        ...
        self.writer.addInstantiation(compName=f"prech_signal_module",
                                         compInsName=f"prech_signal_module_i",
                                         portsPairs=portsPairs2)
        ...
        self.writer.addInstantiation(compName=f"Trivium_DRP  #(.output_bits({rng_nr}))",
                                         compInsName=f"Trivium_DRP_i",
                                         portsPairs=portsPairs3)
